...
from videomind.constants import GROUNDER_PROMPT, PLANNER_PROMPT, VERIFIER_PROMPT, REVISER_PROMPT, REWRITER_PROMPT
...
from videomind.utils.parser import parse_query, parse_span, parse_reviser_response, parse_rewriter_response
...
if __name__ == '__main__':
    ...
    for i in nncore.ProgressBar(range(len(annos))):
        ...
        '''if do_reviser:
            ...
            messages = [{
                ...
                'content': [{
                    ...
                    'type': 'text',
                    'text': REVISER_PROMPT.format(question_and_options=question_and_options, response=dump['answerer_response'])
                }]
            }]
            ...
            #             "detok": [tok.decode([i]) for i in ids],
            ...
            #     " Correct", "Wrong"
            # ]
            # report = probe_token_ids(cands)
            # for k, v in report.items():
            #     print(
            #         f"{k:10s} -> ids={v['ids']}  tokens={v['tokens']}  detok={v['detok']}  single={v['single_token']}")
            #
            #
            # correct_id = None
            # wrong_id = None
            #
            # for s in [" Correct", "Correct"]:
            #     if report.get(s, {}).get("single_token"):
            #         correct_id = report[s]["ids"][0]; break
            # for s in [" Wrong", "Wrong"]:
            #     if report.get(s, {}).get("single_token"):
            #         wrong_id = report[s]["ids"][0]; break
            #
            # print("Picked Correct ID:", correct_id, "Picked Wrong ID:", wrong_id)

            with torch.inference_mode():
                probs_last = model(**data).logits[0, -1].softmax(dim=-1)
            #
            # # assert correct_id is not None and wrong_id is not None, "Correct/Wrong 没有单 token 形式，请改用序列打分"
            # score = (probs_last[correct_id] - probs_last[wrong_id]).sigmoid().item()
        ...
        if reviser_response_tuple[0] == "Correct":
            ...
            reviser_response = parse_reviser_response(response)
        if reviser_response == "Yes":
            print(f"reviser reponse: {reviser_response}")
        elif reviser_response == "No":
            print(f"reviser reponse: {reviser_response}")
	    """
            print('=============== rewriter ===============')
            ...
            dump['answerer2_response'] = response
            dump['response'] = response

        else:
            raise Exception("No Correct or Wrong")
