...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} ASCEND_RT_VISIBLE_DEVICES=${GPULIST[$IDX]} python mvrr/eval/infer_auto.py \
        --dataset $dataset \
        --split $split \
        --pred_path $pred_path \
...
python mvrr/eval/eval_auto.py $pred_path --dataset $dataset
