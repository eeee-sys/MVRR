...
if __name__ == '__main__':
    ...
    tab_ans_all = [SafeInt(0) for _ in range(len(thres) + 5 + 1)]
    ...
    for path in pred_paths:
        ...
        for sample in data:
            ...
            for t in task:
                ...
                if t not in tab_ans:
                    tab_ans[t] = [SafeInt(0) for _ in range(len(thres) + 5 + 1)]
            ...
            if 'answerer2_response' in sample:
                for t in task:
                    tab_ans[t][-1] += 1
                tab_ans_all[-1] += 1
    ...
    if cgbench_mode:
        ...
        tab = tabulate(
            [[task, tab_ans[task][0], tab_ans[task][1]] +
             [f'{tab_ans[task][i] / tab_ans[task][0] * 100:.2f}' for i in range(2, 5)] + [tab_ans[task][-1]]
             for task in tasks if task in tab_ans] +
            [['all', tab_ans_all[0], tab_ans_all[1]] +
             [f'{tab_ans_all[i] / tab_ans_all[0] * 100:.2f}' for i in range(2, 5)] + [tab_ans_all[-1]]],
            headers=['Task', '#Samples', 'Failed', 'Acc', 'Acc (IoU >= 0.5)', 'Acc (IoP >= 0.5)', 'Reviser counts'],
            tablefmt='pretty',
            stralign='left')
        nncore.log(tab)
