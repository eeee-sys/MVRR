...
# 这个类相当于裸的decoder-only transformer
...
# 这个类就是给原始的 Qwen2-VL 模型加了一层外壳，强制输入必须是embedding，确保训练时embedding向量可以反向传播
class AgentQwen2VLForConditionalGeneration(Qwen2VLForConditionalGeneration):
    ...
    def __init__(self, config):
        ...
        # self.lm_head就是把最后的隐状态映射回词表概率分布
