...
from mvrr.constants import VERIFIER_PROMPT
from mvrr.dataset.hybrid import DATASETS
from mvrr.utils.parser import parse_span
...
class VerifyingDataset(Dataset):
    ...
    """
    这个方法是只在训练的时候使用。
    它首先是只取TOP-5的候选视频片段，与真实的span计算
    lou，然后计算positive标签（论文里面对每个片段标注yes或no），然后返回标注好的样本
    """
    @classmethod
